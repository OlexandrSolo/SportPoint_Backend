...
export const clubTrainerSchema = Joi.object({
    ...
    profileImage: Joi.string().uri().required().messages({              // URL зображення профілю
        "any.required": "Фото профілю є обов'язковим полем."
    }),
    contacts: Joi.object({
        phone: Joi.string().pattern(patternLines.PHONE).required().messages({
            "string.pattern.base": "Телефон має містити тільки цифри (може починатися з '+').",
    ...
    type: Joi.string().valid(...typeList).required().messages({           // Ключове поле, щоб визначити тип
...
export const updateClubTrainerSchema = clubTrainerSchema.fork(
    Object.keys(clubTrainerSchema.describe().keys), (schema) => schema.optional());
