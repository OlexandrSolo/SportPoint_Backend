...
export const updateWorkoutPlan = async (req, res) => {
  ...
  const updatedWorkout = await workoutPlanServices.updateWorkoutPlanService(
    id,
    user._id,
    body,
  );
...
export const deleteWorkoutPlan = async (req, res) => {
  ...
  const deletedWorkoutPlane =
    await workoutPlanServices.deleteWorkoutPlanService(id, user._id);
...
export const getWorkoutPlan = async (req, res) => {
  ...
  const plan = await workoutPlanServices.getWorkoutPlanById(id);

  res.json({
    status: 200,
    message: 'Successfully find workout plan',
    data: plan,
  });
};
